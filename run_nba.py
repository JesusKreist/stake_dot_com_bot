...
def main():
    ...
    # Step 1: Scrape props
    if not run_command(f"{venv_python} stake_nba_scraper.py", "Scraping today's NBA props from Stake.com"):
        sys.exit(1)
    
    # Step 2: Analyze
    if not run_command(f"{venv_python} nba_comprehensive_analyzer.py", "Analyzing props against historical data (7-game lookback)"):
        sys.exit(1)
    
    # Step 3: Generate tickets
    if not run_command(f"{venv_python} nba_ticket_generator_4games.py", "Generating 5 diverse tickets"):
        sys.exit(1)
